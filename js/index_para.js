...
pmapS['HFT'] = 'HFTp';
pmapS['OOE'] = 'OOEp';
pmapS['HILO'] = 'HILp';							// 6-10
pmapS['FGS'] = 'FGSp';
pmapS['HCSP'] = 'HCSPp';
pmapS['THFP'] = 'THFPp';
pmapS['HHAD'] = 'HHAp';
pmapS['AlupHHAD'] = 'AHHAp';					// 11-15
pmapS['AlupCRS'] = 'ACRSp';
pmapS['HFMP6'] = 'HFMP6p';
pmapS['AlupOOE'] = 'AOOEp';
pmapS['AlupHILO'] = 'AHILp';
...
pmapS['CrossPoolHHAD'] = 'XHHADp';
pmapS['CrossPoolHDC'] = 'XHDCp';
pmapS['InPlayBetting'] = 'InPlay';
pmapS['InPlaySpecial'] = 'InPlaySpecial';		// 36-40
pmapS['AlupFGS'] = 'AFGSp';
pmapS['FHAD'] = 'FHADp';
pmapS['AlupFHAD'] = 'AFHADp';
pmapS['CrossPoolFHAD'] = 'AXFHADp';
pmapS['CrossPoolHILO'] = 'AXHILp';				// 41-45
...
pmapS['FHLO'] = 'FHILp';
pmapS['AlupFHLO'] = 'AFHILp';
pmapS['CrossPoolFHLO'] = 'AXFHILp';
pmapS['FCRS'] = 'FCRSp';
pmapS['AlupFCRS'] = 'AFCRSp';
pmapS['CrossPoolFCRS'] = 'AXFCRSp';
pmapS['AlupNTS'] = 'ANTSp';
pmapS['CrossPoolNTS'] = 'AXNTSp';
pmapS['CHLO'] = 'CHLp';
pmapS['AlupCHLO'] = 'ACHLp';
pmapS['CrossPoolCHLO'] = 'AXCHLp';
...
function IsBetTypeEnabled(betType) {
...
function IsAllUpEnabled(betObj) {
  if (betObj.type.indexOf('ALUP')>=0 && betObj.family=='FB' && GetFBPara('CrossPoolBet')!='1')
    return false;
  else if (betObj.type.indexOf('ALUP')>=0 && betObj.family=='HB' && GetPara('HorseRaceCrossPool')!='1')
    return false;
  return true;
}

function IsAllUpBetlineEnabled(betObj) {
  var tmp = betObj.betline.split('/');
  var firstBetType = '';
  var isSingleAlup = true;
  for (var i = 1; i < tmp.length; i++) {
    var betlineTmp = tmp[i].split(' ');
    if (betlineTmp[0].indexOf(']') >= 0)
      continue;
    if (i == 1) {
      firstBetType = betlineTmp[0];
    }
    else {
    	if (firstBetType != betlineTmp[0] && !(isCwinPool(firstBetType) && isCwinPool(betlineTmp[0])) || (isCwinPool(firstBetType) && isCwinPool(betlineTmp[0]))) { // implied it is cross pool all up
        isSingleAlup = false;
        if ( betObj.family == 'HB' && (!GetXPoolAllUpEnabled(firstBetType) || !GetXPoolAllUpEnabled(betlineTmp[0])) )
          return false;
        else if (betObj.family == 'FB' && ((IsValidFBBetType(firstBetType) && !GetXPoolAllUpEnabled(firstBetType))
          || (IsValidFBBetType(betlineTmp[0]) && !GetXPoolAllUpEnabled(betlineTmp[0]))))
          return false;
      }
    }
  }
  if (isSingleAlup && betObj.family == 'FB' && IsValidFBBetType(firstBetType) && GetFBPara('Alup' + firstBetType) != '1')
    return false;
  return true;
}
...
function IsExtraTimeBetEnabled(isExtraTime) {
  if ( isExtraTime=='1' ) {
...
function isCwinPool(type) {
	if ($j.inArray(type, ["CWA", "CWB", "CWC"]) >= 0) {
		return true;
	}
	return false;
}
