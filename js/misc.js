...
function autoSwitchToPoolingMode(){
	//var engineFrame = top.document.getElementById('push_engine');
	//var infoFrame = top.document.getElementById('info');
	if (engineStatus == "STREAMING" || engineStatus == "POLLING") {
		totalTime = 0;
		//engineFrame.contentWindow.polling = false;
		oddsRefreshType = "push";
		$(".refresh").hide();
	}else if ( engineStatus == "STALLED" || engineStatus == "DISCONNECTED" ) {
	     oddsRefreshType = "pull";
		 $(".refresh").show();
	}else{
		totalTime++;
		if(totalTime > statusTimeout){
		    //engineFrame.contentWindow.polling = true;
		    oddsRefreshType = "pull";
		    $(".refresh").show();
		   // infoFrame.contentWindow.location.href = infoFrame.contentWindow.location.href;
		}
    }
    setTimeout("autoSwitchToPoolingMode()", 1000);
}

function autoSwitchToPullingModeForRacing() {
    //var engineFrame = top.document.getElementById('push_engine');
    //var infoFrame = top.document.getElementById('info');
    if (engineStatus == "STREAMING" || engineStatus == "POLLING") {
        totalTime = 0;
        ...
        if (totalTime > statusTimeout) {
            //engineFrame.contentWindow.polling = true;
            if (top.statusChanged != "changed") {
                top.document.getElementById('info').contentWindow.location.href = top.document.getElementById('info').contentWindow.location.href;
                top.statusChanged = "changed";
            }
            top.status = "poll";
            //$(".refresh").show();
    ...
    setTimeout("autoSwitchToPullingModeForRacing()", 1000);
}
...
function initializePushPage(miscPath, showStatus, onEngineReady, onEngineLost) {
  ...
  var domain = top.document.getElementById('info') ? top.document.getElementById('info').contentWindow.GetGlobalResources("Domain", "js") : null;
//  if (typeof (GetGlobalResources) == "undefined") {
//      
//  } else {
//      domain = GetGlobalResources("Domain", "js");
//  }
...
function initializeEngine(pushPage, lsLibPath, onStatusChange, existingMasterRef) {
  ...
  if (existingMasterRef == null) {
      pushPage.onEngineCreation = function(lsEngine) {
          var host = top.document.getElementById('info') ? top.document.getElementById('info').contentWindow.GetGlobalResources("PushServer", "js") : null;
          var port = top.document.getElementById('info') ? top.document.getElementById('info').contentWindow.GetGlobalResources("PushServerPort", "js") : null;
...
function attachCallbacksAndStatusManager(pushPage, miscPath, showStatus, onEngineReady, onEngineLost) {
  ...
  pushPage.onClientError = function(mex) {
      ...
      if (mex == "Unable to find the Engine. Retrying. If the problem persists, please check your configuration.") {
          alert(infoFrame.contentWindow.getClientMsg());
      }
      else if (mex == "") {
      }
      else {
