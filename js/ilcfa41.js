...
function autoRefreshIlcMatchesByPush() {
    ...
    page = initializePushPage("http://bet.hkjc.com/info/include/js/commons/custom/", oddsPushIconDisplay, onEngineReady, onEngineLost);
    initializeEngine(page, "http://bet.hkjc.com/info/include/js/commons/lightstreamer/", null, masterWinRef);
...
function convertToDateTime(str) {
    if (str != "" && str != null) {
        str = str.replace(/^\s+|\s+$/, "");
        if (isDateTime(str)) {
            var tmpArrDate = str.split(" ")[0].split("http://bet.hkjc.com/");
...
function sortMultipleLines(a, b) {
    ...
    var isSplitLineA = a.line.indexOf('http://bet.hkjc.com/') >= 0;
    var lineA = parseFloat(a.line);
    var isSplitLineB = b.line.indexOf('http://bet.hkjc.com/') >= 0;
