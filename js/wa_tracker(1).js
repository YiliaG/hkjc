...
/************************** Configuration Codes, manage by certified cosultant*************************/
if (typeof s_account == 'undefined'){
	s_account = "hkjcweb-prd";
	cdomain = 2;
}
...
function s_doPlugins(s) {
	if(!s.pageName){
		try {
		s.pageName=unescape(s.getPageNameFrame().toLowerCase())
		s.prop6=parent.document.referrer; //get referrer of framed page
		s.prop17=s.pageURL=parent.document.location.href;//set full URL for framed page
	}
		catch(err){
			s.pageName=unescape(s.getPageName().toLowerCase())
			s.prop6=document.referrer; //set page referrer
			s.prop17=document.location.href; //set full URL
		}
	}
	if(!s.hier1){	
		s.hier1=s.pageName;	
	...
	/* internal campaign */
	if(!s.eVar11){
	s.eVar11=s.prop11=s.getQueryParam('b_cid');
		if(!s.eVar11){
			s.eVar11=s.prop11=location.hash.substr(1);
		}
	}
	s.temp = s.pageName.substr(0, s.pageName.lastIndexOf('/'));
	if(!s.prop1) s.prop1=s.eVar1=s.temp.split('/')[1];
	if(!s.prop2) s.prop2=s.eVar2=s.temp.split('/')[2];
	if(!s.prop3) s.prop3=s.eVar3=s.temp.split('/')[3];
	if(!s.prop4) s.prop4=s.eVar4=s.temp.split('/')[4];
	if(!s.prop5) s.prop5=s.eVar5=s.temp.split('/')[5];
	if(!s.prop12) s.prop12=s.eVar12=s.temp.split('/')[6];
	if(!s.prop13) s.prop13=s.eVar13=s.temp.split('/')[7];
	if(!s.prop14) s.prop14=s.eVar14=s.temp.split('/')[8];
	if(!s.prop15) s.prop15=s.eVar15=s.server; //set site domain
	if(!s.eVar6)s.eVar6=s.pageName; //set pageName to custom conversion report
	if(!s.prop8)s.prop8=s.pageName; //Event_ID|PageName
	
	/* Enhanced download tracking */
	var sc_fileurl=s.downloadLinkHandler();
	if(sc_fileurl){
		s.eVar18=s.prop8=sc_fileurl;
		s.linkTrackVars="prop8,eVar6,eVar18,events";
...
/* ----------------------------------------------- */
	...
	function trackClickEvent(event_id){	
		...
		s.linkTrackVars='prop8,eVar6,eVar9,prop9,prop10,eVar10,'+sc_event;
